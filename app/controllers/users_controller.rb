class UsersController < ApplicationController
  ...
  def friends
    if @user == current_user || current_user.admin?
      @request_friends = @user.request_friends.order("friendships.created_at desc")
      @inverse_request_friends = @user.inverse_request_friends.order("friendships.created_at desc")
      ...
      @friends = @user.friends.order("friendships.updated_at desc") + @user.inverse_friends.order("friendships.updated_at desc") # 兩類各排各的，依 updated_at 是 status 改為 true 才正式成立朋友關係
